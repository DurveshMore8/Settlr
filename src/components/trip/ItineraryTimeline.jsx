...
const ItineraryTimeline = ({ itinerary, isAdmin, tripStatus, onEdit, onDelete, onAddClick }) => {
    ...
    return (
        <div className="relative space-y-6 before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-purple-500/50 before:via-white/5 before:to-transparent">
            {sortedItinerary.map((item) => {
                ...
                return (
                    <div key={item._id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
                        <div className="flex items-center justify-center w-10 h-10 rounded-full border border-white/10 bg-slate-900 group-hover:border-purple-500/50 transition-all md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10">
                            <ItemIcon size={18} className="text-purple-400" />
                        </div>
                        <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-white/5 border border-white/10 p-6 rounded-3xl group-hover:bg-white/[0.08] transition-all">
                            <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-2">
                                    <span className="px-2 py-0.5 bg-purple-500/20 text-purple-400 text-[9px] font-black uppercase tracking-widest rounded-full">{item.type}</span>
                                    {item.dateTime && (
                                ...
                                {isAdmin && tripStatus === 'active' && (
                                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
                                        <button
                                            onClick={() => onEdit(item)}
                                            className="p-1.5 text-slate-500 hover:text-purple-400 rounded-lg hover:bg-purple-500/10"
                                        >
                                            <Edit2 size={14} />
                                        </button>
                                        <button
                            ...
                            {item.address && <p className="text-slate-500 text-xs font-medium mt-2 flex items-start gap-1.5"><MapPin size={12} className="shrink-0 mt-0.5 text-slate-700" />{item.address}</p>}
                        </div>
                    </div>
                );
            })}
